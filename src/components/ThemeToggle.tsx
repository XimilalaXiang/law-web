...
const ThemeToggle: React.FC = () => {
  ...
  return (
    ...
    >
      {isDark ? (
        <Sun className="h-5 w-5 text-yellow-400 group-hover:text-yellow-500 transition-colors" />
      ) : (
        <Moon className="h-5 w-5 text-gray-600 group-hover:text-blue-600 transition-colors" />
      )}
    </button>
  );
};
